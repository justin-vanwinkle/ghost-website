...
StorageConnStr ?= $(shell stty -echo; read -p "Azure Storage Connection String: " pwd; stty echo; echo $$pwd)
...
run-bash:
	docker run -it --rm --name ghost-test -e url=http://localhost:3001 -e dbPassword=$(DBPASSWORD) -e AZURE_STORAGE_CONNECTION_STRING="$(StorageConnStr)" -e environment=development -p 3001:2368 justinreg.azurecr.io/website:latest /bin/bash
